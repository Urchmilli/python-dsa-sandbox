...
@time_it
def binary_search(list_of_element, no_to_find):
    left_index = 0
    right_index = len(list_of_element) - 1
    middle_index=0

    while left_index <= right_index:
        middle_index = (left_index + right_index) // 2
        middle_num = list_of_element[middle_index] 

        if middle_num == no_to_find:
            return middle_index
...
def find_all_occurances(numbers, number_to_find):
    ...
    #find indices on th left hand side
    i = index - 1
    while i >=0:
